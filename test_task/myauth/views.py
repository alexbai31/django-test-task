...
from templatetags.test_tags import is_allowed

from templatetags.test_tags import bizzfuzz
...
def export_excel(request):
    ...
    writer.writerow(['Username', 'Birthday', 'Random Number', 'Allowed', 'BizzFuzz'])
    for u in user:
        writer.writerow([u.username, u.birthday, u.random_number, is_allowed(u.birthday), bizzfuzz(u.random_number)])
...
def user_add(request):
    ...
    if request.method == "POST":
        if request.POST.get('add_button') is not None:
            ...
            data = {'username': 'username'+str(random.randint(1, 2000)),
